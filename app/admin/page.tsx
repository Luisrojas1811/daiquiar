...
export default function AdminPage() {
  ...
  const [categories, setCategories] = useState(['Bodies', 'Catsuits', 'Conjuntos'])
  const [message, setMessage] = useState('')
  const [copy, setCopy] = useState({ title: 'Moda que inspira tu estilo.', description: 'Indumentaria femenina seleccionada con amor. Diseños únicos, calidad y envíos a todo el país.', footer: 'HEY BBY! Somos Daiquiar. Diseño y calidad a precios únicos. CABA, Buenos Aires. Envíos a todo el país.' })

  function saveCopy(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    const data = new FormData(event.currentTarget)
    const next = { title: String(data.get('title')), description: String(data.get('description')), footer: String(data.get('footer')) }
    setCopy(next)
    window.localStorage.setItem('daiquiar-copy', JSON.stringify(next))
    setMessage('Textos actualizados en la tienda pública')
  }
